...
RUN pip install -r test_requirements.pip && \\
    ansible-playbook $$ANSIBLE_OPTIONS -i inventory test.yml -e rds_security_groups=${RDS_SGS} -e rds_subnet_group_name=${RDS_SUBNET_GROUP} -e rds_vpc=${RDS_VPC}
endef
export DOCKER_BODY
